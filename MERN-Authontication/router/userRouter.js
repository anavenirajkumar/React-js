...
router.post('/register',[
 ...
 check('password').isLength({min: 6}).withMessage('Please Register Here'),  //// Direct Register Click Cheste Error Message Alert
 ...
 */
 ...
 router.post('/login', [
   check('email').isEmail().withMessage('Enter a proper Email'),
   check('password').isLength({min: 6}).withMessage('Invalid Credentials ') //// Direct Login Click Cheste Error Message Alert
 ], async (request, response) => {
          /// login logic ///
          let errors = validationResult(request);
          if(!errors.isEmpty()){
